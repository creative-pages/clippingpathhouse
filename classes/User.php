...
class User {
	...
	public function userRegister($data) {
		...
		$real_country = $this->fm->validation($data['real_country']);
		...
		if ($fld_fullname && $fld_email && $fld_password && $reapet_password && $fld_country) {
			...
			if ($email_check_result == NULL) {
				if ($fld_password == $reapet_password) {
					$fld_passwords = SHA1(MD5($fld_password));

					$sql = "INSERT INTO `tbl_user_info`(`fld_fullname`, `fld_email`, `fld_phone`, `fld_address`, `fld_country`, `real_country`, `fld_website`, `fld_payment_method`, `fld_payment_policy`, `fld_password`, `fld_source`, `fld_photo`, `fld_ipaddress`) VALUES ('$fld_fullname', '$fld_email', '', '', '$fld_country', '$real_country', '', '', '', '$fld_passwords', '$fld_source', '', '$fld_ipaddress')";
					$result = $this->db->insert($sql);
					if ($result) {

						//---------------Email sender---------------

						$recipient = $this->db->recipient_email; //recipient 
						...
						$mail_body .= "Real Country: $real_country \r\n";
						...
						$client_mail_body .= "Welcome to CPH Graphics Media.\r\n";
        				...
        				$client_mail_body .= "Email: $admin_mail\r\n";
        				$client_mail_body .= "Web: www.clippingpathhouse.com\r\n";
        				$client_mail_body .= "Skype: CPH Graphics Media\r\n";
        				$client_mail_body .= "Call: +88-01741906076\r\n";
        				$client_mail_body .= "548, Mirpur-14, Kafrul\r\n";
        				$client_mail_body .= "Dhaka- 1206, Bangladesh\r\n";
						

						$subject = "Thanks for your registration at CPH Graphics Media"; //subject 
				...
				// 		return '<div class="alert alert-success">Account created successfully.</div>';
						...
						$force_login = "SELECT * FROM `tbl_user_info` WHERE `fld_email` = '$email'";
            			$force_login_result = $this->db->select($force_login);
            			if ($force_login_result != false) {
            				$user_details = $force_login_result->fetch_assoc();
		...
		if (!empty($email)) {
			...
			if ($result != false) {
				...
				if ($update_result) {
					...
					$mail_body .= "Clipping Path House Graphics Media Team\r\n\n";
					$mail_body .= "CPH Graphics Media\r\n\n";
					$mail_body .= "Further information please contact\r\n";
					$mail_body .= "$admin_mail\r\n";
					$mail_body .= "Skype: CPH Graphics Media\r\n";
					
					$subject = "Password_Reset_(Confidential)"; //subject 
					$header = "From: CPH_Graphics_Media_Admin"; //optional headerfields 
	...
	public function updateUser($data) {
		...
		$fld_payment_method = $this->fm->validation($data['fld_payment_method']);
		$fld_payment_policy = $this->fm->validation($data['fld_payment_policy']);
			...
			if ($file_name) {
				if ($file_size > 2097152) {
			     ...
			     .implode(', ', $permited)."</div>";
			    	...
			    	$sql = "UPDATE `tbl_user_info` SET `fld_fullname`='$fld_fullname',`fld_phone`='$fld_phone',`fld_address`='$fld_address',`fld_website`='$fld_website', `fld_payment_method` = '$fld_payment_method', `fld_payment_policy` = '$fld_payment_policy',  `fld_photo`='$image' WHERE `id` = '$user_id'";
				...
				$sql = "UPDATE `tbl_user_info` SET `fld_fullname`='$fld_fullname',`fld_phone`='$fld_phone',`fld_address`='$fld_address',`fld_website`='$fld_website', `fld_payment_method` = '$fld_payment_method', `fld_payment_policy` = '$fld_payment_policy' WHERE `id` = '$user_id'";
				$result = $this->db->update($sql);
				if ($result) {
					return '<div class="alert alert-success">Account updated successfully.</div>';
				} else {
