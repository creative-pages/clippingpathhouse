...
class Service {
	...
	public function singleServiceJob($table_name, $id) {
		$id = $this->fm->validation($id);

		$sql = "SELECT * FROM `$table_name` WHERE `fld_user_id` = '$id'";
		$result = $this->db->select($sql);
		return $result;
	}
	...
	public function updateSeenServiceByClient($table_name, $key, $value, $user_id) {
		$key = $this->fm->validation($key);
		$value = $this->fm->validation($value);
		$user_id = $this->fm->validation($user_id);

		$sql = "UPDATE `$table_name` SET `$key` = '$value' WHERE `fld_user_id` = '$user_id'";
		$result = $this->db->select($sql);
		return $result;
	}
