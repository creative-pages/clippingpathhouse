...
class Job {
	...
	public function updateRegisterJob($data) {
		...
		$fld_charge = $this->fm->validation($data['fld_charge']);

		$update_id = $this->fm->validation($data['update_id']);

		if ($fld_payment_type && $fld_amount) {
			$sql = "UPDATE `tbl_register_job` SET `fld_total_files` = '$fld_total_files', `fld_amount` = '$fld_amount', `fld_charge` = '$fld_charge', `fld_payment_type` = '$fld_payment_type', `fld_payment_status` = '$fld_payment_status' WHERE `id` = '$update_id'";
