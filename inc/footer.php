...
<div class="top_to">
  ...
  <i class="">
      <img src="assets/img/Navigate-up-icon.png" alt="" class="w-100 h-100" >
  </i>
</div>
<!---- to-top end ---->
...
<!-- free_trial and get_quote part start -->
<section style="background-color: #2c3c62; border-bottom: 2px outset #FFA235;" class="py-5">
    <div class="container">
        <h2 class="text-uppercase mb-4" style="color: #f15f3c; font-weight: 900; font-size: 65px;">NOT SURE WHERE TO START ?</h2>
        <div class="row">
            <div class="col-md-7">
                <p class="text-white text-capitalize mb-4" style="font-size: 22px; letter-spacing: 1px;">Talk to one of our customer service representatives who will review your needs & give you perfect solutions for your sepecific business reality & needs.</p>
                <div class="call_email">
                    ...
                    <a href="tel:+8801711442709" class="rounded text-center text-decoration-none mr-2">
                        <img src="assets/img/call-us-now.png" alt="" height="80" width="100" >
                    </a>
                    <a href="mailto:info@clippingpathphoto.com" class="rounded text-center text-decoration-none mr-2">
                        <img src="assets/img/send-mail.png" alt="" height="80" width="100" >
                    </a>
                </div>
                <br>
                <div class="call_email_2nd">
                    <a href="free-quote.php" class="mb-4 mb-md-0 free_trial btn btn-primary btn-lg">Free Quote</a>
                    <a href="free-trial.php" class="mb-4 mb-md-0 free_trial_purple btn btn-lg py2 px-3 font-weight-normal">Free Trial</a>
                    <a href="authentication.php" class="mb-4 mb-md-0 free_trial btn btn-primary btn-lg">Place Order</a>
                </div>
            </div>
            <div class="col-md-5 position-relative">
                <img src="assets/img/home/clipping path service provider company.png" class="not_sure d-none d-lg-block" height="520" style="bottom: -48px; right: 0;">
            </div>
        </div>
    </div>
</section>
<!-- free_trial and get_quote part end -->
				...
				<form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST">
					...
					<div class="form-row mb-2">
						<div class="col-6">
			                ...
			                $q_num2 = rand(1,9);
			                ?>
			                <input type="hidden" id="q_num1" value="<?= $q_num1; ?>">
			                <input type="hidden" id="q_num2" value="<?= $q_num2; ?>">

			                <input type="text" id="q_captcha_fill" value="<?= $q_num1 . '+' . $q_num2; ?>" class="form-control text-center" readonly="">
						</div>
						<div class="col-6">
							<input type="number" name="captcha" id="q_captcha" class="form-control text-center" required="">
						</div>
					</div>
					<input type="submit" name="quick_query" id="quick_query" class="btn btn-success btn-sm" disabled="disabled">
				</form>
				<?= isset($query_result) ? $query_result : ''; ?>
			</div>
		</div>
	</div>
	<br>
...
<script src="assets/js/owl-slider/owl.carousel.min.js"></script>
...
$(document).mouseover(function(){
	...
	if ($('#q_captcha').val() == '') {
		$('#quick_query').attr('disabled', 'disabled');
	}
});
$(document).ready(function() { 
	$("#captcha").keyup(function(){
    	var num1 = $('#num1').val();
    	var num2 = $('#num2').val();
    	var sumation = parseInt(num1, 10) + parseInt(num2, 10);
	...
	$("#q_captcha").keyup(function(){
    	var q_num1 = $('#q_num1').val();
    	var q_num2 = $('#q_num2').val();
    	var q_sumation = parseInt(q_num1, 10) + parseInt(q_num2, 10);

    	var q_captcha = $('#q_captcha').val();

    	if (q_captcha == q_sumation) {
    		$('#quick_query').removeAttr('disabled', 'disabled');
    	} else {
    		$('#quick_query').attr('disabled', 'disabled');
    	}
	});
	...
	if(inputLocalFont) {
	    ...
	    var totalfile=fileList.length;
			if(totalfile>3){
				alert("Your Total file exceed. Maximum 3 file you can upload.");
				$('#userImage').val('');
				return false;
			}
	    var anyWindow = window.URL || window.webkitURL;
	        var maxfullsize=47185920;
			...
			if(single>=maxsize){
					alert("File too large. File must be less than 15 megabytes");
					$('#userImage').val('');
					return false;
				}
			}
			if(allsize>=maxfullsize){
				alert("All Image Size Must be Less Then 45MB!!!");
				$('#userImage').val('');
				return false;
			}
    ...
    $('#uploadForm').submit(function(e) {	
        if($('#userImage').val()) {
            ...
            return false; 
			 ...
			 e.preventDefault();
			 alert("You can't select Any Files");
			}
    });
    ...
    <script>
			function file_count(){
				if(document.getElementById('files_table').rows.length > 3){
					alert('Only 3 files allowed');
					document.getElementById('files_table').innerHTML = '';
				}
			}
			...
			function file_dir(){
				...
				var dater = "<?php echo gmstrftime("%d_%m_%Y_%H.%M.%S");?>";
				var histor = dater+"_"+name;
                                  //$.session.set('myVar', histor); 
				document.getElementsByName('txt_file_dir')[0].value = histor;
			}
			function upload_check(){
				document.getElementsByName('txt_check')[0].value = '1';
			}
		</script>
		<script>
